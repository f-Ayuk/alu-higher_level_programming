...
if (args.length === 0 || args.length === 1) { // check if the array is empty or has only one element
  ...
  const nums = args.map(arg => parseInt(arg)); // map the array of arguments to an array of integers using the parseInt function
  const max = Math.max(...nums); // find the maximum value in the array using the Math.max function with the spread operator
  let secondMax = -Infinity; // initialize a variable to store the second maximum value, starting from -Infinity
  for (const num of nums) { // use a for-of loop to iterate over the array of integers
